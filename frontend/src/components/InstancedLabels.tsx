...
export default memo(function InstancedLabels({
  ...
  // Pre-compute agent ids (avoid recomputing per tick)
  const agentIds = useMemo(
    () => buildings.slice(0, count).map((b) => b.agent_id),
    [buildings, count]
  );
  ...
  useFrame(({ camera, clock }) => {
    ...
    if (now - lastLabelTick.current > 0.12) {
      ...
      if (hidden) {
        ...
        const hasFocus = !!(focusedId != null || focusedBId != null);
        ...
        if (hasFocus) {
          ...
          for (let i = 0; i < count; i++) {
            const id = agentIds[i];
            const isFocused = id === focusedId || id === focusedBId;

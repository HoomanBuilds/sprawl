...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // P&L tint: blend the agent tint color with the theme face color.
    // vTint.a is the blend strength (green = profitable, red = losing, gray = neutral).
    if (vTint.a > 0.01) {
      // Detect face pixels (background between windows) vs window pixels
      float isFacePixel = step(length(wallColor - uFaceColor), 0.08);
      vec3 blendedTint = mix(uFaceColor, vTint.rgb, vTint.a);
...
export default memo(function InstancedBuildings({
  ...
  const { uvFrontData, uvSideData, riseData, tintData, roofColorData } = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // P&L tint (rgb = color, a = blend strength). Sourced from city-layout
      // computeBuildingTint: green = profitable, red = losing, gray = neutral.
      const t = b.tint;
      if (t && t.length === 4) {
        tint[i * 4 + 0] = t[0];
        tint[i * 4 + 1] = t[1];
  ...
  useEffect(() => {
    if (!material.uniforms) return;
    const idA = focusedBuilding != null ? agentToIdx.get(focusedBuilding) : undefined;
    const idB = focusedBuildingB != null ? agentToIdx.get(focusedBuildingB) : undefined;
  ...
  }, [focusedBuilding, focusedBuildingB, dimAll, agentToIdx, material]);
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < count; i++) {
      arr[i] = liveAgentIds?.has(buildings[i].agent_id) ? 1.0 : 0.0;
    }
    liveAttr.needsUpdate = true;
  }, [liveAgentIds, buildings, count]);
